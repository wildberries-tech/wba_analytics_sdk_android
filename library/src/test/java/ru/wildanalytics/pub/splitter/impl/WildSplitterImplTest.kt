...
@Suppress("UnusedFlow")
internal class WildSplitterImplTest {

    private lateinit var repository: SplitterPropertiesRepository
    private lateinit var splitter: WildSplitterImpl
    private val config = WildSplitterConfig(
        apiKey = "apiKey",
        fetchUrl = "fetchUrl",
        userId = "userId"
    )
    private var onConfigChangedFlags: OnConfigChangedFlags = OnConfigChangedFlags()
    private val getOnConfigChangedFlags: (oldConfig: WildSplitterConfig, newConfig: WildSplitterConfig) -> OnConfigChangedFlags =
        { _, _ -> onConfigChangedFlags }

    @BeforeEach
    fun setUp() {
        repository = mockk(relaxed = true)
        splitter = WildSplitterImpl(
            repository,
            config,
